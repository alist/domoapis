...
function ResponseExecutor(resHandler) {
  ...
  this.done = function() {
    ...
    if (!_.isObject(resHandler.meta)) {
      hasErrors ? resHandler.code(DEFAULT_ERROR_CODE) : resHandler.code(DEFAULT_SUCCESS_CODE);
    }

    resHandler.renderView = hasErrors ? resHandler.resErrView : resHandler.resView;
